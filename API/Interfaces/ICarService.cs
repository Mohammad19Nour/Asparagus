...
{
    public Task<(Car? car, string Message)> AddCar(NewCarDto carDto,string city); 
    ...
    public Task<(bool Success,string Message)> MakeBooking( int userId, DateTime startTime,string city);
    public Task<(int? CarId,string Message)> GetAvailableCarId(DateTime startTime, DateTime endTime);
    public  Task<List<List<(DateTime Start,DateTime End)>>> GetAvailableDates(string city);
}